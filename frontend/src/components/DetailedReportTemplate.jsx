...
export default function DetailedReportTemplate({ reportData, language = 'en' }) {
  ...
  return (
    <div className="pdf-report-container">
      ...
      {/* PAGE 1: COVER PAGE */}
      <div className="pdf-page relative p-12 bg-white flex flex-col justify-between border-[8px] border-amber-600/30" style={{ height: '297mm', width: '210mm', pageBreakAfter: 'always', position: 'relative' }}>
        <div className="text-center pt-8">
          <span className="text-xs uppercase tracking-widest font-bold text-amber-700 font-serif">
            {t('omGanesha')}
          </span>
        ...
        <div className="text-center space-y-6 my-auto">
          <GaneshaSVG />
          <h1 className="font-serif text-4xl font-extrabold text-gray-900 tracking-wider uppercase leading-snug">
            {t('coverTitle')}<br />
            <span className="text-amber-700 text-3xl font-bold">{t('coverSubtitle')}</span>
          </h1>
      ...
      <div className="pdf-page relative p-12 bg-white flex flex-col justify-between" style={{ height: '297mm', width: '210mm', pageBreakAfter: 'always' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          {renderHeader(8)}
          <h2 className="font-serif text-2xl font-bold text-gray-900 mb-3">{t('bhagyankCareerTitle')}</h2>
          <div className="space-y-3 text-xs text-gray-700 leading-relaxed text-justify">
            <div className="p-4 bg-amber-50/50 border border-amber-200 rounded-xl space-y-1.5">
              <strong className="text-amber-700 uppercase tracking-wider block text-[10px]">✦ {t('professionalChannelsLabel')}</strong>
              <p className="text-gray-900 font-semibold">{getTranslation(currentBhagyank.career)}</p>
            </div>

            <p>{t('bhagyankDescText')}</p>

            <div className="p-4 bg-sky-50/40 border border-sky-200 rounded-xl">
              <strong className="text-sky-700 uppercase tracking-wider block text-[10px] mb-2">✦ {language === 'en' ? 'Ideal Career Fields' : (language === 'gu' ? 'આદર્શ કારકિર્દી ક્ષેત્રો' : 'आदर्श करियर क्षेत्र')}</strong>
              <div className="grid grid-cols-2 gap-1.5">
                {getTranslation(currentBhagyank.career).split(',').slice(0, 8).map((c, i) => (
                  <span key={i} className="text-[10px] text-gray-600 flex items-center gap-1"><span className="text-sky-500">▸</span> {c.trim()}</span>
                ))}
              </div>
            </div>

            <div className="p-4 bg-violet-50/40 border border-violet-200 rounded-xl">
              <strong className="text-violet-700 uppercase tracking-wider block text-[10px] mb-1">✦ {language === 'en' ? 'Financial Prosperity Guidance' : (language === 'gu' ? 'આર્થિક સમૃદ્ધિ માર્ગદર્શન' : 'आर्थिक समृद्धि मार्गदर्शन')}</strong>
              <p className="text-gray-700">
                {language === 'en'
            ...
            <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
              <strong className="text-gray-600 uppercase tracking-wider block text-[10px] mb-1">✦ {language === 'en' ? 'Business Partnership Advice' : (language === 'gu' ? 'વ્યापारिक ભागીદારી સলાહ' : 'व्यापारिक साझेदारी सलाह')}</strong>
